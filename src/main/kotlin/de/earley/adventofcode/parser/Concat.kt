...
import de.earley.adventofcode.empty
 ...
 */
fun <I, O> many(parser : Parser<I, O>, delimiter : Parser<I, *>? = null) : Parser<I, List<O>> = recursive { many ->
    val empty = epsilon<I, List<O>>(emptyList())
    // 1 , 2 , 3
    val one = parser.map { listOf(it) }
    val moreParser = if (delimiter == null) parser + many else (parser + delimiter.void() + many)
    val more = moreParser.map { head, tail ->
        listOf(head) + tail
    }
