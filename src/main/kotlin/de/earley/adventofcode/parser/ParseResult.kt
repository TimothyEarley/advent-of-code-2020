...
sealed class ParseResult<out I, out O> {
    sealed class Ok<O> : ParseResult<Nothing, O>() {
        ...
        data class Maybe<I, O>(val ok : Ok<O>, val error : ErrorData<I>) : Ok<O>()
    }
...
fun <I, A, B> ParseResult<I, A>.flatMap(f : (A) -> ParseResult<I, B>) : ParseResult<I, B> = when (this) {
    ...
    is ParseResult.Ok.Maybe<*, A> -> when (val result = this.ok.flatMap(f)) {
        ...
        is ParseResult.Ok.Maybe<*, B> -> {
            ...
            (ParseResult.Ok.Maybe(
                result.ok,
                ErrorData.Multiple.from(listOf(result.error as ErrorData<I>, error as ErrorData<I>))
            ))
        }
        is ParseResult.Error -> {
            @Suppress("UNCHECKED_CAST") // I does not change
            (ParseResult.Error(
                ErrorData.Multiple.from(
                    listOf(
                        result.error,
                        error as ErrorData<I>
                    )
                )
            ))
        }
...
fun <I> List<ParseResult.Error<I>>.combineErrors() : ParseResult.Error<I> =
    ParseResult.Error(ErrorData.Multiple.from(this.map { it.error }))
